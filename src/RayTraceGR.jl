...
function minkowski(x::Chain{V,1,T}) where {V, T}
    D = ndims(V)
    CT = fulltype(Chain{V,1,T})
    Chain{V,1}(SVector{D,CT}((bitsign(i==1) * Chain{T}(Chain(v(i))) for i in 1:D)...))
end
...
"""
function dmetric(metric::Metric, x::Chain{V,1,T}) where {Metric, V,T}
    D = ndims(V)
    DT = Chain{V,1,T}
    TDT = Dual{T,fulltype(DT)}
    xdx = Chain{V,1,TDT}(SVector{D,TDT}(
        TDT(x[1], Chain{V,1}(SVector{D,T}(1, 0, 0, 0))),
        TDT(x[2], Chain{V,1}(SVector{D,T}(0, 1, 0, 0))),
        TDT(x[3], Chain{V,1}(SVector{D,T}(0, 0, 1, 0))),
        TDT(x[4], Chain{V,1}(SVector{D,T}(0, 0, 0, 1)))))
    gdg = metric(xdx)
    g = Chain{V,1}(SVector{D,fulltype(DT)}((
        Chain{V,1}(SVector{D,T}((
            gdg.v[b].v[a].val for a in 1:D)...))
        for b in 1:D)...))
    dg = Chain{V,1}(SVector{D}((
        Chain{V,1}(SVector{D}((
            Chain{V,1}(SVector{D,T}((
                gdg.v[c].v[b].eps[a] for a in 1:D)...))
            for b in 1:D)...))
        for c in 1:D)...))
    g, dg
end
...
#TODO         Pixel{T}(x, u, zero(SVector{3,T}))
#TODO     end
